...
def register():
    ...
    if not (request.json and request.json.get("username") and request.json.get("password")
            and request.json.get("name") and request.json.get("email")):
        return jsonify({"message": 'Please, provide "name", "email", "username" and "password" in body.'}), 400
    ...
    if EmployeeModel.find_by_username(username, to_dict=False):
    ...
    new_user = EmployeeModel(
        username=username, name=name, is_admin=is_admin, email=email,
...
def login():
    ...
    username = request.json["username"]
    password = request.json["password"]
    current_user_ = EmployeeModel.find_by_username(username, to_dict=False)
    ...
    if EmployeeModel.verify_hash(password, current_user_.hashed_password):
        access_token = create_access_token(identity=username, additional_claims=groups)
        refresh_token = create_refresh_token(identity=username, additional_claims=groups)
        return {
...
def update_password():
    ...
    user = EmployeeModel.find_by_email(email, to_dict=False)
    if not user:
        return jsonify({"message": "User not found."}), 404
    if new_password:
        if user.verify_hash(password, user.hashed_password):

            user.hashed_password = EmployeeModel.generate_hash(new_password)
            user.save_to_db()
